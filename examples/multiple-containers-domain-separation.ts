...
const UserRepositoryToken =
	interfaceToken<IUserRepository>()("IUserRepository");
...
const OrderRepositoryToken =
	interfaceToken<IOrderRepository>()("IOrderRepository");
...
class OrderService {
	...
	async createOrder(orderData: CreateOrderDto): Promise<Order> {
		...
		this.logger.log(
			`Order created with ID: ${order.id}, Total: $${order.total}`,
		);
...
const PaymentGatewayToken =
	interfaceToken<IPaymentGateway>()("IPaymentGateway");
...
class PaymentService {
	...
	async processPayment(order: Order): Promise<PaymentResult> {
		this.logger.log(
			`Processing payment for order ${order.id}: $${order.total}`,
		);
 ...
 */
function createUserDomainContainer(
	sharedContainer: ReturnType<typeof createSharedContainer>,
) {
	return (
		new ContainerBuilder()
 ...
 * It borrows shared singletons without taking ownership of them.
 */
function createOrderDomainContainer(
	sharedContainer: ReturnType<typeof createSharedContainer>,
) {
	return (
		new ContainerBuilder()
...
class ECommerceApplication {
	...
	async initialize(): Promise<void> {
		...
		this.orderDomainContainer = createOrderDomainContainer(
 ...
 */
async function simulateHttpRequestHandlers(
 ...
 */
async function simulateCrossDomainWorkflow(
	app: ECommerceApplication,
): Promise<void> {
	...
	try {
		...
		console.log(
			`✅ Step 3 - Payment processed: ${paymentResult.success ? "Success" : "Failed"}`,
		);
	} finally {
		orderScope.dispose();
	}
