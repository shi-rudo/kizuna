...
const rootContainer = new ContainerBuilder()
	...
	.registerSingletonFactory("SupportedLanguages", () => [
		"en",
		"es",
		"fr",
		"de",
...
// Test event bus
eventBus.on("user-registered", (user: any) =>
	console.log(`- Event received: User ${user.name} registered`),
);
...
console.log(
	`- Same Logger instance (singleton): ${scope1.get("Logger") === scope2.get("Logger")}`,
);
