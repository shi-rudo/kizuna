...
| Factory callbacks receive a resolution-only context. | [Factory context tests](../tests/factory-resolution-context.test.ts) and [public type tests](../tests/public-api-hardening.test-d.ts) | Factory code remains trusted and can use references from another source. |
...
| Disposal cleans owned values in dependency-aware order and aggregates errors. | [Disposal order tests](../tests/disposal-order.test.ts), [ownership tests](../tests/disposal-ownership.test.ts), [concurrency tests](../tests/async-disposal-concurrency.test.ts), and [disposal error tests](../tests/disposal-errors.test.ts) | Transient values are untracked, and independent async branches have no completion order. Async cleanup has a finite concurrency limit. |
