...
- `build()` checks its options at runtime and throws the new `InvalidBuildOptionsError` (code `INVALID_BUILD_OPTIONS`, fields `option` and `value`) for options or a `diagnostics` option that is not an object, an unsupported `validation` mode, a `diagnostics.listener` that is not a function, or an unsupported `diagnostics.level`. Only `undefined` selects a default; `null` is an unsupported value. TypeScript already rejects these values. Before, JavaScript code that passed an unknown `validation` mode got eager validation without an error.
